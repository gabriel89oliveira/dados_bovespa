...
class SeedHistoricPrices:

    storage_path = "storage/bovespa/historic_price/"

    def update():

        for year in list(range(2010, datetime.now().year + 1)):

            file_path = SeedHistoricPrices.storage_path + 'COTAHIST_A' + str(year) + '.txt'

            if( exists(file_path) ):
                print('  |- Fazendo seed: ' + str(year), flush=True)
                df = SeedHistoricPrices.read_file(file_path)
                SeedHistoricPrices.seed_database(df)

            else:
                print('  |- Nenhum arquivo encontrado: ' + str(year), flush=True)


    """
    Read file from storage
    with all historic prices
    ...
    """
    Seed database with DataFrame
    created and filtered from file
    ...
    def seed_database(df):
        ...
        while (len(df.index) > 0):
            ...
            HistoricPrices.insert_many(list_of_dicts).on_conflict(action='IGNORE').execute()
